...
from common.cnn_mlp import compile_cnn
...
def create_model(trial, input_shape):
    ...
    filters = trial.suggest_categorical("filters", [4, 8, 16, 32, 64])
...
def objective(trial, X_train, y_train, X_val, y_val, X_test, y_test, study_name):
    ...
    model_path = (
        f"results/optuna/cnn/models/{study_name}_model_trial{trial.number}.keras"
    )
    ...
    # Save model history
    history_path = (
        f"results/optuna/cnn/history/{study_name}_history_trial{trial.number}.json"
    )
...
if __name__ == "__main__":
    ...
    storage_path = f"sqlite:///results/optuna/cnn/251123_partitions_gridsearch_M{M}_optuna.db"
