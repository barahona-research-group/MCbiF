...
# import MCF class and utility functions
from mcbif.mcf import MultiscaleClusteringFiltration
from mcbif.io import load_results, save_results
from mcbif.utils import _get_partition_clusters, _cluster_id_preprocessing


class MultiscaleClusteringBifiltration:
    """Main class to construct MCbiF from a sequence of partitions and analyse its multiparameter persistent homology."""

    def __init__(self, method="standard", max_dim=2):
        """Initialise MCbiF object.
        ...
        # initialse 0 and 1 conflict measures
        self.conflict_0 = None
        self.conflict_0_avg = None
        self.conflict_1_diff = None
        self.conflict_1_avg = None
    ...
    def load_data_from_file(self, file_path):
        ...
        self.conflict_0 = mcbif_results["h"]
        self.conflict_0_avg = mcbif_results["h_bar"]
        self.conflict_1_diff = mcbif_results["c"]
    ...
    def _compute_mcbif_bigrades_standard(
        ...
        if precomp_mcf is not None:
            ...
            mcf = MultiscaleClusteringFiltration(max_dim=self.max_dim, method="standard")
            mcf.load_data(
    ...
    def _compute_mcbif_bigrades_nerve(
        ...
        if precomp_mcf is not None:
            ...
            mcf = MultiscaleClusteringFiltration(max_dim=self.max_dim, method="nerve")
            mcf.load_data(
                partitions=self.partitions, filtration_indices=self.filtration_indices
            )
            mcf.build_filtration(tqdm_disable=tqdm_disable)
    ...
    def compute_conflict_1_measures(self):
        ...
        return self.conflict_1_diff, self.conflict_1_avg
    ...
    def compute_all_measures(
        ...
        mcbif_results["conflict_0"] = self.conflict_0
        mcbif_results["conflict_0_avg"] = self.conflict_0_avg
        mcbif_results["conflict_1_diff"] = self.conflict_1_diff
        mcbif_results["conflict_1_avg"] = self.conflict_1_avg
    ...
    def plot_conflict_1_diff(self, path=None, title=None):
        """Plot persistent 1-conflict difference."""
        # shade lower diagonal part
        shade_lowtri = np.ones_like(self.conflict_1_diff)
        shade_lowtri[np.tril_indices_from(shade_lowtri, k=0)] = np.nan
        shade_lowtri = shade_lowtri.T

        c_abs = np.nanmax(abs(self.conflict_1_diff))
    ...
    def plot_conflict_0(self, path=None, title=None):
        """Plot persistent 0-conflict."""
        # shade lower diagonal part
        shade_lowtri = np.ones_like(self.conflict_0)
        shade_lowtri[np.tril_indices_from(shade_lowtri, k=0)] = np.nan
        shade_lowtri = shade_lowtri.T
        ...
        # plot h
        fig, ax = plt.subplots(1, figsize=(7, 7))
        im = ax.imshow(self.conflict_0, vmin=0, vmax=1, extent=extent, interpolation="none")
